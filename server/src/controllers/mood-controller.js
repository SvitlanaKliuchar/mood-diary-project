...
export const createMood = async (req, res, next) => {
  try {
    ...
    if (existingEntry) {
      return res
        .status(400)
        .json({ error: "You already have a mood entry for this day." });
    }
...
export const updateMood = async (req, res, next) => {
  try {
    ...
    const parsedData = createMoodSchema.parse({
      ...
      emotions: req.body.emotions
        ? JSON.parse(req.body.emotions).map(String)
        : existingMood.emotions,
      sleep: req.body.sleep
        ? JSON.parse(req.body.sleep).map(String)
        : existingMood.sleep,
      productivity: req.body.productivity
        ? JSON.parse(req.body.productivity).map(String)
        : existingMood.productivity,
    });
...
export const deleteMood = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Mood entry deleted successfully!" });
  } catch (err) {
