...
function combineFactors(moodRecord) {
  ...
  return [...(emotions || []), ...(sleep || []), ...(productivity || [])];
}
...
const calculateMoodChartData = (entries) => {
  ...
  return lastSeven.reverse().map((entry) => ({
    date: entry.date,
    ...
    if (activities && activities.length) {
      activities.forEach((activity) => {
        moodActivityMap[mood][activity] =
          (moodActivityMap[mood][activity] || 0) + 1;
      });
    }
  });
...
const processNotes = (notes, allMoodNotes = null) => {
  ...
  const filteredTokens = tokens.filter(
    (token) => !stopwords.words.includes(token),
  );
  ...
  filteredTokens.forEach((token) => {
    ...
    if (!stemToOriginal[stemmed]) {
      ...
      if (
        stemToOriginal[stemmed].count <
        filteredTokens.filter((t) => stemmer.stem(t) === stemmed && t === token)
          .length
      ) {
        stemToOriginal[stemmed].word = token;
        stemToOriginal[stemmed].count = filteredTokens.filter(
          (t) => stemmer.stem(t) === stemmed && t === token,
        ).length;
      }
    }
  });
