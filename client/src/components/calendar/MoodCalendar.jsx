...
const MoodCalendar = () => {
  const { entries, displayedDate, setDisplayedDate } = useContext(EntriesContext);
  ...
  const dayMoods = useMemo(() => {
    ...
    entries.forEach((entry) => {
      map[format(new Date(entry.date), "yyyy-MM-dd")] = entry.mood.toLowerCase();
    });
    return map;
  ...
  return (
    <div className={styles["calendar-container"]} style={{ position: "relative" }}>
