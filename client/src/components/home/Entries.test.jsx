  ...
  default: vi.fn(({ isOpen, onClose, title, message, actionText, onAction }) => (
    isOpen ? (
      <div data-testid="notification-modal">
        ...
        <button onClick={onAction} data-testid="action-button">{actionText}</button>
      </div>
    ) : null
  )),
}));
...
describe('Entries Component', () => {
  ...
  const renderComponent = (
    ...
    const entriesContextValue = { ...mockEntriesContext, ...entriesContextOverrides };
    const loadingContextValue = { ...mockLoadingContext, ...loadingContextOverrides };
    ...
    it('re-fetches entries when displayedDate changes', async () => {
      ...
      rerender(
        <AuthContext.Provider value={mockAuthContext}>
          <EntriesContext.Provider value={{ ...mockEntriesContext, displayedDate: newDate }}>
  ...
  describe('UI Rendering', () => {
    ...
    it('formats date and time correctly', async () => {
      ...
      Date.prototype.toLocaleDateString = vi.fn().mockImplementation(function(locale, options) {
        if (this.toISOString().includes('2023-05-01')) {
          return 'May 1, 2023';
        }
        return 'Unknown date';
      });
      Date.prototype.toLocaleTimeString = vi.fn().mockImplementation(function(locale, options) {
        if (this.toISOString().includes('2023-05-01')) {
          return '12:00 PM';
        }
        return 'Unknown time';
  ...
  describe('Notification Feature', () => {
    it('shows notification when entries count reaches 5', async () => {
      const mockEntries = Array(4).fill().map((_, i) => ({
        id: i + 1,
        mood: 'great',
      ...
      rerender(
        <AuthContext.Provider value={mockAuthContext}>
          <EntriesContext.Provider value={{ ...mockEntriesContext, entries: updatedEntries }}>
            <LoadingContext.Provider value={mockLoadingContext}>
              <Entries />
            </LoadingContext.Provider>
          </EntriesContext.Provider>
    ...
    it('closes notification when close button is clicked', async () => {
      const mockEntries = Array(5).fill().map((_, i) => ({
        id: i + 1,
        mood: 'great',
        date: `2023-05-0${i+1}T12:00:00Z`,
        emotions: []
