...
vi.mock('./form-elements/SubmitButton.jsx', () => ({
  default: () => <button data-testid="submit-button" type="submit">Submit</button>,
}));
...
describe('EntryForm Component', () => {
  ...
  const renderComponent = (isEditing = false) => {
    ...
    return render(
      <EntriesContext.Provider value={{ addEntry: mockAddEntry, updateEntry: mockUpdateEntry, entries: mockEntries }}>
  ...
  describe('Validation', () => {
    it('shows validation error when form is submitted without required fields', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Please select both date and mood.')).toBeInTheDocument();
      });
  ...
  describe('Form Submission - Create Mode', () => {
    it('successfully submits a new entry', async () => {
      ...
      fireEvent.change(screen.getByTestId('note-input'), { target: { value: 'New test note' } });
      fireEvent.click(screen.getByTestId('submit-button'));
      await waitFor(() => {
        expect(mockStartLoading).toHaveBeenCalled();
    ...
    it('handles generic error during submission', async () => {
      ...
      fireEvent.click(screen.getByTestId('submit-button'));
      await waitFor(() => {
        expect(screen.getByText('An error occured while submitting your entry')).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Form Submission - Edit Mode', () => {
    it('successfully updates an existing entry', async () => {
      renderComponent(true);
      fireEvent.change(screen.getByTestId('note-input'), { target: { value: 'Updated note' } });
      fireEvent.click(screen.getByTestId('submit-button'));
      await waitFor(() => {
        expect(mockUpdateEntry).toHaveBeenCalledWith('123', expect.objectContaining({
          append: expect.any(Function),
          get: expect.any(Function)
        }));
        expect(mockNavigate).toHaveBeenCalledWith('/home');
