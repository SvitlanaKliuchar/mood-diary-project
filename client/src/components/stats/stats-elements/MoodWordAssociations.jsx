...
const MoodWordAssociations = ({ moodWordAssociations }) => {
  ...
  const getWordColor = (tfidf) => {
    ...
    const maxTfidf = currentMoodWords.length
      ? Math.max(...currentMoodWords.map((w) => w.tfidf))
      : 1;
  ...
  const getWordStyle = (word) => {
    ...
    const maxValue = currentMoodWords.length
      ? Math.max(...currentMoodWords.map((w) => w.value))
      : 1;
  ...
  return (
    <>
      <div className={styles["mood-word-controls"]}>
        <div
          className={`${styles["select-mood-icon"]} ${styles["select-mood-icon-2"]}`}
        >
          <select
            name="mood-select"
            id="mood-select"
      ...
      {selectedMood &&
      moodWordAssociations &&
      moodWordAssociations[selectedMood] ? (
        view === "words" ? (
          <div className={styles["mood-words-container"]}>
            <h4>
              Words Associated with{" "}
              <span className={styles["mood-title"]}>{selectedMood}</span> Mood
            </h4>
            ...
            <div className={styles["word-legend"]}>
              <p>
                Larger words = more frequent. More vibrant colors = more
                distinctive to this mood.
              </p>
            </div>
          </div>
        ) : (
          <div className={styles["word-pairs-container"]}>
            ...
            <div className={styles["word-pairs-list"]}>
              {moodWordAssociations[selectedMood].coOccurring.map(
                (pair, idx) => (
                  <div key={idx} className={styles["word-pair-item"]}>
                    <div className={styles["word-pair"]}>
                      <span className={styles["pair-word"]}>
                        {pair.words[0]}
                      </span>
                    ...
                    <div className={styles["pair-count"]}>{pair.count}x</div>
                  </div>
                ),
              )}
            </div>
