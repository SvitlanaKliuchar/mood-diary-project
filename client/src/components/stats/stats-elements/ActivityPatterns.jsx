...
const ActivityPatterns = ({ activityPatterns }) => {
  ...
  const validMoods = Object.keys(activityPatterns).sort(
  ...
  const findActivityIcon = (activity) => {
    ...
    return allActivityOptions.find(
      (option) => option.value.toLowerCase() === activity.toLowerCase(),
    )?.iconUrl;
  };
              ...
              {activityPatterns[selectedMood].map((item, index) => {
                ...
                return (
                  <div key={index} className={styles["activity-count"]}>
                    ...
                    <span className={styles.count}>({item.count}x)</span>
                  </div>
                );
              })}
            </div>
